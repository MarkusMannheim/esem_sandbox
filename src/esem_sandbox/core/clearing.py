...
which is the price it clears at by default: both sides accept a published reference
price rather than negotiating. Making the two sides find a price between them is
available as an extension, but the anchor is the path everything is written from,
...
def cara_certainty_equivalent(payoffs: np.ndarray, weights: np.ndarray,
    ...
    The frame is shifted to the worst cell. The identity holds for any shift, and
    only this one is numerically safe: shifting by the minimum makes every exponent
    non-positive, so the worst cell contributes exactly one, better cells underflow
    harmlessly towards zero, and the result is bounded below by the worst cell.
    Shifting by the maximum instead makes the largest exponent ``a`` times the
    spread, and a distribution holding one year of rent at the value of lost load
    then overflows to infinity and returns a certainty equivalent of minus
    infinity.

    Weights must be a probability distribution, which this checks. Weights summing
    to anything else shift the answer by ``ln(sum)/a`` without raising: at a fifth of
    the weight and this model's coefficients that is $134,000 per MW-year, enough to
    return a certainty equivalent above the largest payoff in the distribution and to
    hand a producer a negative hurdle.
    """
...
def cap_anchor(cost_basis_per_mwh: float, realised_mean_excess_per_mwh: float,
    ...
    not the risk the writer carries. Using it once lifted a cap anchor to about
    $2,000/MWh on the strength of a single scarce interval.
    """
...
def clear_bilateral(settings: Settings, roster, fleet, history: list[dict[str, float]],
    ...
    for retailer in retailers:
        ...
        for block in settings.blocks():
            ...
            # One rung of the ladder, so the divisor is the LADDER LENGTH and not
            # this contract's remaining life. They are the same number every tick
            # but the bootstrap, which clears three times at tenors three, two and
            # one to open on a laddered book. Dividing by the tenor there wrote the
            # rungs at a third, a half and the whole of the target, so a run opened
            # at 1.833 times the cover a retailer wanted rather than at 1.0, and
            # every producer's exposure and every early hurdle came off that.
