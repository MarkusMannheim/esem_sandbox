...
Three things about it carry the result.
...
35 per cent are each tested against their own costs, and the capacity-factor gap
that shows up in dispatch stays visible there instead of leaking into a bid.

The certainty equivalent is exact rather than a variance approximation. A
...
def build_size_mw(peak_mw: float, tech: TechCost, settings: Settings) -> float:
    ...
    settings.toml and tech_costs.csv where a reader can see and change them.
    """
...
def build_ceiling_mw(peak_mw: float, tech: TechCost, settings: Settings) -> float:
    ...
    gigawatt system dominates a 23 gigawatt one, and on the high growth
    path a fixed one bound in every year of a 20-year run, so the model was
