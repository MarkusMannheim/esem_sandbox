...
def default_roster() -> tuple[Agent, ...]:
    ...
    The risk aversions are a chosen ordering rather than a measurement, and the
    ordering is what matters: gentailers, which own both load and plant and are
    naturally hedged by the pairing, sit below merchants, which are not. The regional
    merchant is the representative new entrant and is the least risk averse of the
    producers, because it is the one deciding whether to build. The levels are
    stylised, and KNOWN_LIMITATIONS.md records what turns on them.
    """
    return (
