...
Three anchors, at four, eight and 12 years ahead. A plant decided today
...
class Cell:
    ...
    over cells is weighted by it, without exception. The failure to guard against is
    computing these weights, passing them everywhere, and then averaging the cells
    without them, which enters a one-in-ten-year peak at one in three.
    """
...
def renormalised(cells: tuple[Cell, ...]) -> tuple[Cell, ...]:
    ...
    Taking part of the lattice is legitimate, and anything wanting a faster run should
    be able to, but a subset of a
    normalised set is not normalised, and every reduction downstream assumes it is.
    Nine of the 45 cells sum to a fifth, which is enough to return a
...
def dispatch_anchor(settings: Settings, fleet: tuple[Unit, ...], bundle: dict,
    ...
    for cell in cells:
        ...
        outcomes.append(CellOutcome(
            ...
            # one deep hour and a gap that is 50 shallow ones need very different
...
def update_projected_entry(state: EntryState, anchors: list[Anchor],
    ...
    discrete tranches: on this fleet a peaker's rent falls $30,000
    ...
    in the past. Where nothing can earn its cost the state falls to zero and
    ...
    for anchor in anchors:
        ...
        # number of candidates - on this fleet, 15 gigawatts of assumed entry
        # on a 12 gigawatt system, wandering and never settling. A market adds
...
def interpolated_rent(anchors: dict[int, float], offset: float,
    ...
    years of a 25-year life would cost a third of the forward's runtime
    to move a discounted number very little.

    After the last anchor the terminal applies as a step, not a fade. The step is
    the honest shape: the model does not know less and less about years 13
    through 25, it knows nothing about any of them, and a fade would dress
    that up as a trend.
    """
    if not anchors:
        return terminal
...
def lifetime_rent_by_cell(anchors: list[Anchor], tech: TechCost) -> np.ndarray:
    ...
    rent at four years is the same one that produced it at 12. Averaging over
