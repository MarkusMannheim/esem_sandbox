...
This is why it is modelled: a target that is easy in one year and impossible
...
class SchemeRow:
    ...
    def sought(self, year: int, held_mw: float = 0.0) -> float:
        """What the scheme has to buy this year to stand on its milestone.

        A milestone is a stock. The trajectory rises to 9,000 MW, so a scheme that
        already holds 4,000 buys 2,500 to reach 6,500 rather than another 6,500.
        Reading each milestone as that year's purchase turns a 9,000 MW target into
        21,500 MW of buying.
        """
        return max(0.0, float(self.milestones.get(year, 0.0)) - held_mw)
...
def summarise(row: SchemeRow, year: int, sought: float, lines: list[AwardLine],
    ...
    # A year with no milestone keeps whatever channel it came in with. Zero awarded
    # against zero sought otherwise satisfies the test and reports a target met in
    # the 16 years of a 20-year run that never set one.
    if sought > 0 and awarded >= sought - 1e-9:
        binding = MET
...
def clear_scheme(row: SchemeRow, bids: list[Bid], year: int,
                 held_mw: float = 0.0) -> tuple[SchemeYear, list[AwardLine]]:
    ...
    of them bound first.
    """
    sought = row.sought(year, held_mw)
    if sought <= 0:
