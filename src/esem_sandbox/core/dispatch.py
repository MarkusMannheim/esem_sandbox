...
def _balance_level(day: np.ndarray, power: float, rte: float) -> np.ndarray:
    ...
    is a contradiction rather than a schedule. Finding it is one bisection, because
...
def _run_storage(units: list[Unit], price_of, residual_mw: np.ndarray,
    ...
    never exceed the pre-storage peak, so a store cannot make the peak worse. The
    ...
    # years: a 20-year run ends with more storage rows than it started with,
    ...
    for (duration, rte), members in sorted(groups.items(), key=lambda kv: -kv[0][0]):
        ...
        # The 12-hour pumped hydro cannot charge for 15 hours and discharge
        # for 12 inside one day, so its charge level was pushed above its
        # discharge level, the day was rejected as incoherent, and the unit sat idle
        # for all 365 days of the year while the model
        ...
        # first costs two reductions instead of a 20-step search, and it is
        ...
        # The viability test gates buying, never selling. A store only charges when
        # the day's spread covers the round trip, so a lull with no cheap hours leaves
        # it empty, which is the behaviour the worst-week chart exists to show. What
        # it already holds it may always deliver.
        #
        # Gating selling as well would bind in the hour that matters most. On the
        # hottest day of the drought year the overnight load is high enough that a
        # store cannot charge, so the day fails the spread test, and a store that
        # already carries a reserve would be barred from discharging it. More storage
        # would then raise the peak rather than lower it. What limits discharge is the
        # state of charge, and that is enforced below.
        ...
        # late trough at 11 - and a store balanced across the day as a whole
        # then delivers in the evening energy it will not store until that night.
        # Checking totals cannot see that; checking the running total can.
        #
        # An hour that asks for more than the store holds is capped at what it
        # holds, and the rest of the day proceeds. Capping cannot break the peak
        # guarantee: less discharge returns net load towards its pre-storage level
        # and never above it, and less charging only deepens a trough.
        #
        ...
        soc = 0.0
        excursion = np.cumsum(charge * rte - discharge, axis=1)
        high = excursion.max(axis=1)         # furthest above the day's opening charge
        low = (-excursion).max(axis=1)       # furthest below it
        net = excursion[:, -1]
        for d in range(n_days):
            # A day that stays inside both limits needs no repair, and testing that
            # with two scalars is cheaper than walking its hours.
            if high[d] <= energy_cap - soc and low[d] <= soc:
                soc += net[d]
                continue
            c_row = charge[d].tolist()
            d_row = discharge[d].tolist()
            for h in range(HOURS_PER_DAY):
                c = c_row[h]
                if c > 0.0:
                    room = (energy_cap - soc) / rte
                    if c > room:
                        c = room if room > 0.0 else 0.0
                        c_row[h] = c
                    soc += c * rte
                out = d_row[h]
                if out > 0.0:
...
def _apply_ladder(residual: np.ndarray, stack_price: np.ndarray,
    ...
    """Demand response, call-hour budgets and the administered cap.

    Sequential in time because both the tranche budgets and the rolling threshold
    ...
    # Demand response is priced like anything else that can meet an hour, so a
    # tranche offering below the dearest plant is called before it. Paying a customer
    # $300 to stop is cheaper than burning distillate at $480, and a ladder stacked
    # strictly above every generator makes the price fall as the system tightens.
    #
    # Plant and tranches therefore go into one merit order, and the price each hour
    ...
    if prices is None or caps is None or not len(prices):
        ...
        caps = np.array([firm_capacity])
    plant_price = np.asarray(prices, dtype=float)
    plant_cap = np.asarray(caps, dtype=float)
    tier_price = np.array([t.price_per_mwh for t in tiers], dtype=float)
    tier_cap = np.array([t.capacity_mw for t in tiers], dtype=float)

    # One ordered list of every offer, with the tranches marked so their call-hour
    # budgets can be counted against them.
    merit_price = np.concatenate([plant_price, tier_price])
    merit_cap = np.concatenate([plant_cap, tier_cap])
    is_tier = np.concatenate([np.zeros(len(plant_price), dtype=int) - 1,
                              np.arange(len(tiers))])
    order = np.argsort(merit_price, kind="stable")
    merit_price, merit_cap, is_tier = (merit_price[order], merit_cap[order],
                                       is_tier[order])
    cheapest_tier_base = float(
        merit_cap[(is_tier < 0) & (merit_price <= tier_price.min())].sum()
    ) if len(tiers) else firm_capacity

    # Fast path: if no hour reaches even the cheapest tranche and the rolling sum
    # can never reach the threshold, the sequential pass has nothing to do.
    if residual.max() <= cheapest_tier_base:
    ...
    for h in range(len(residual)):
        need = residual[h]
        if need > cheapest_tier_base:
            used_by_tier = np.zeros(len(tiers))
            filled = 0.0
            hour_price = price[h]
            for j in range(len(merit_price)):
                if live[j] <= 0:
                    continue
                take = min(need - filled, live[j])
                if take <= 0:
                    break
                filled += take
                i = int(is_tier[j])
                if i >= 0:
                    used_by_tier[i] += take
                hour_price = merit_price[j]
                if filled >= need - 1e-9:
                    break
            for i, used in enumerate(used_by_tier):
                if used > 0:
                    ladder_mw[h] += used
                    remaining_hours[i] -= 1
...
def dispatch_year(settings: Settings, year: int, demand_mw: np.ndarray,
    ...
    # One stack, used twice: the hydro schedule prices its water against it, and the
    # year is then priced off it. Building it again gives the same three arrays.
    ...
    for u in hydro_units:
        schedule, unit_water = _hydro_schedule(
            ...
            u.energy_budget_gwh * 1000.0, prices, caps, floor)
    ...
    # residual does not depend on the price, so there is no fixed point to chase. A
    # schedule chosen by ranking prices would need one, because it moves the price it
    # ranked.
    ...
    price, unserved, ladder_mw, administered = _apply_ladder(
        net_residual, stack_price, firm_capacity, settings, prices, caps)
    ...
    # Demand response that was called sits inside the merit order, so it takes load
    # off the plant rather than adding on top of it. Dispatching against the residual
    # before that subtraction has the plant serving load that a customer agreed not
    # to take, and the year fails to balance by the megawatts the tranche supplied.
    dispatch_residual = net_residual + cut_total - ladder_mw
