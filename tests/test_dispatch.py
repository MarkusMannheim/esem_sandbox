...
def test_the_drought_year_is_the_one_that_breaches_the_standard(settings, bundle):
    ...
    contrast the model is calibrated on, so it is pinned rather than left to chance.
...
def test_storage_does_not_chase_the_price_it_produces(settings, bundle):
    ...
    # And the one channel price does have is the spread gate, not the quantities.
    # With hydro out, the residual storage sees is demand less rooftop and VRE, which
    # no offer touches, so any change here came through that gate.
    no_hydro = replace(settings, fleet=tuple(
        u for u in settings.fleet if u.technology not in ("hydro", "phes")))
    dearer = replace(no_hydro, fleet=tuple(
        replace(u, srmc_per_mwh=u.srmc_per_mwh * 2.0)
        if u.technology in ("coal", "ccgt", "ocgt") else u for u in no_hydro.fleet))
    base = _year(no_hydro, bundle, 4)
    moved = _year(dearer, bundle, 4)
    assert not np.array_equal(base.price, moved.price), "the probe did not move price"

    for unit in (u for u in no_hydro.fleet if u.technology == "battery"):
        x = base.generation_mwh[unit.unit]
        y = moved.generation_mwh[unit.unit]
        differing = int((np.abs(x - y) > 1e-9).sum())
        # A day either cycles or does not, so a change lands on whole days rather
        # than drifting across the year. Anything approaching every hour would mean
        # the quantities themselves were being read off the price.
        assert differing < len(x) * 0.05, (
            f"{unit.unit}'s schedule moved in {differing} hours when only the price "
            "moved. The spread gate should turn days on and off, not reshape the year"
        )
...
def test_a_long_duration_store_is_not_idle_all_year(settings, bundle):
    """Targeting a full cycle every day is impossible for a 12-hour store: it
    cannot charge for 15 hours and discharge for 12 inside one day. Its
...
def test_administered_pricing_releases_rather_than_latching(settings, bundle):
    ...
    fleet = tuple(u if u.technology in variable
                  else replace(u, capacity_mw=u.capacity_mw * 0.85)
                  for u in settings.fleet)
    result = dispatch_year(replace(settings, fleet=fleet), 2026,
                           shape * (12500.0 / shape.max()),
                           bundle["wind_cf"][4], bundle["solar_cf"][4])
    administered = np.asarray(result.administered_hours).astype(bool)
    assert administered.any(), "this fleet is meant to reach the threshold at all"
    first = int(np.argmax(administered))
    assert not administered[first:].all(), (
        "administered pricing latched on at the first breach and never released"
    )

    # And the window it decides on must be a window of SETTLED prices: an hour that
    # went in uncapped and came out capped is what left the residue behind.
    window = int(settings.market["cumulative_price_window_hours"])
    price = np.asarray(result.price)
    apc = settings.market["administered_price_cap_per_mwh"]
    csum = np.concatenate(([0.0], np.cumsum(price)))
    idx = np.arange(len(price))
    roll = csum[idx + 1] - csum[np.maximum(0, idx + 1 - window)]
    quiet = ~administered
    assert not ((roll[quiet] >= settings.hourly_price_threshold)
                & (price[quiet] > apc)).any(), (
        "an hour whose settled window was over the threshold was left uncapped"
    )
    assert price[administered].max() <= apc + 1e-9


@pytest.mark.parametrize("peak", (12500.0, 11000.0, 10000.0, 9500.0))
def test_the_energy_balance_closes_at_every_peak_it_is_claimed_for(
        settings, bundle, peak):
    """The identity above was only ever checked at the packaged peak.

    `--peak` is a shipped command-line knob and the balance does NOT hold all the way
    down it. Rooftop is a fixed 5,200 MW netted off demand and never curtailed, so on
    a small enough system there are hours when rooftop alone exceeds demand by more
    than the utility wind and solar fleet can be cut back to absorb, and the surplus
    is silently dropped. Measured: the first breach is at a peak of about 9,300 MW,
    and by 7,000 MW it is 808 hours and 212 GWh, with the command line reporting
    ...
    """
    for y in range(settings.weather["shape_years"]):
        res = _year(settings, bundle, y, peak=peak)
        served = sum(v for k, v in res.generation_mwh.items() if k != "rooftop")
        balance = served + res.ladder_mw + res.unserved_mwh - res.operational_demand_mw
        assert np.abs(balance).max() < 1e-6, (
            f"peak {peak:,.0f}, year {y}: worst hourly imbalance "
            f"{np.abs(balance).max():.6f} MW"
        )


def test_a_tighter_hour_is_never_cheaper(settings):
    """Demand response sits in the merit order, not above it.

    A tranche that will stop consuming at $300/MWh is cheaper than a distillate
    peaker offering at $480, so it is called first and the peaker sets the price
    when it is the marginal one. A ladder stacked above every generator instead
    prices the first short megawatt at $300, below the $480 the hour before it
    settled at, so the price falls as the system tightens.
    """
    import numpy as np

    from esem_sandbox.core import dispatch as D

    units = D._in_service(settings, 2026)
    prices, caps, _ = D._offer_stack(units, settings, {"water_value": 0.0})
    firm = float(caps.sum())
    floor = settings.dispatch["must_run_offer_per_mwh"]

    seen = []
    for level in np.linspace(firm - 600.0, firm + 700.0, 400):
        one = np.array([level])
        stack = D._price_from_stack(one, prices, caps, floor)
        # One hour at a time, so a sweep does not exhaust the call-hour budgets.
        price, unserved, ladder, _adm = D._apply_ladder(
            one, stack, firm, settings, prices, caps)
        seen.append((float(price[0]), float(ladder[0]), float(unserved[0])))

    for (before, _l, _u), (after, _l2, _u2) in zip(seen, seen[1:]):
        assert after >= before - 1e-9, (
            f"the price fell from ${before:,.2f} to ${after:,.2f} as the system "
            "got tighter"
        )

    # And the cheapest tranche is called before the plant stack is exhausted, which
    # is what "inside the merit order" means.
    levels = np.linspace(firm - 600.0, firm + 700.0, 400)
    early = [(lvl, ladder) for lvl, (_p, ladder, _u) in zip(levels, seen)
             if lvl < firm and ladder > 0]
    assert early, (
        "no demand response was called until every generator was already running, "
        "so the ladder is still stacked above the merit order rather than inside it"
    )
