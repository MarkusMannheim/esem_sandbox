...
def test_swap_cover_is_weighted_by_the_hours_the_block_covers(settings):
    """A peak-only swap covers six hours in 24. Counting it as though it
...
def test_no_capacity_factor_enters_the_comparison(settings, merchant):
    ...
    35 are each tested against their own costs, not against a common
...
def test_the_ceiling_is_a_number_of_producers_worth_of_plant(settings):
    ...
    fixed megawatt ceiling that damps a 12 gigawatt system dominates a
    23 gigawatt one, and bound in every year of a 20-year run."""
    tech = settings.tech("ocgt")
