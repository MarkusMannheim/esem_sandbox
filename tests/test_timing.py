...
The requirement is that a paired run finishes in minutes rather than hours. This
guards a dispatched year, which is the unit everything else is built from.

A leg costs much more than its dispatched years alone. Candidate storage is valued by
running the daily scheduler for each storage row in every cell at every anchor, which
is more scheduler runs than there are dispatched years, so a projection from this
number alone understates a leg by about half. There is no tighter target than
"minutes", and no timing figure is quoted elsewhere for that reason.
